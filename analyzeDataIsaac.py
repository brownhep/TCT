...
def parseador():
  ...
  parser.add_argument( "-a", "--annealing_step", type = int, required = False, default = -1, help = "Annealing step on which this run was done. Default = %(default)s", metavar = "" )
  parser.add_argument( "-l", "--laser_type", type = str, required = False, default = None, help = "Laser Type used for filenaming purposes. Choices = i, f, or b for 1065nm, 660nm front, or 660nm back respectively", metavar = "" )
  parser.add_argument( "-t", "--temperature", type = int, required = False, default = -1, help = "Temperature of run. Default = %(default)s", metavar = "" )
...
def makeFilename (filename, annealingStep, laserType, temperature):
  i = 0
  nameCutoff = len(filename) - 1
  while i < len(filename):
    if filename[i] == '_':
      nameCutoff = i
      break
    i += 1
  fullname = filename[0:nameCutoff] + '_Ann' + str(annealingStep) + '_'
  if laserType == 'i':
    fullname += 'IR_'
  if laserType == 'f':
    fullname += 'RedFront_'
  if laserType == 'b':
    fullname += 'RedBack_'
  fullname += str(temperature)
  return fullname
  

def main( files, noiseFile, bulk, annealingStep, laserType, temperature, integrationWindow, verbosity ):
  results = []
  dataTCT = []
  ...
  fullFilename = makeFilename(str(dataTCT[-1].filename), annealingStep, laserType, temperature)
  writeResults( "results.txt", "Voltage(V)\tCharge(C)\tPairs\tMIPS\n", results )
  writeResults( "TCT_results/" + fullFilename + "_results.txt", "Voltage(V)\tCharge(C)\tPairs\tMIPS\n", results)

if __name__ == "__main__":
  args = parseador()
  main( args.files, args.noise_file, args.bulk, args.annealing_step, args.laser_type, args.temperature, args.integration_window, args.verbosity )
